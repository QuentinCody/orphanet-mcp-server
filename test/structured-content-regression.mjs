...
/**
 * Regression tests for orphanet-mcp-server structuredContent responses.
 */
...
if (index) {
  assertContains('src/index.ts', index, 'OrphanetDataDO', 'index exports OrphanetDataDO');
...
if (codeMode) {
  assertContains('src/tools/code-mode.ts', codeMode, 'orphanet_search', 'code-mode registers orphanet_search');
  assertContains('src/tools/code-mode.ts', codeMode, 'orphanet_execute', 'code-mode registers orphanet_execute');
}

if (queryData) {
  assertContains('src/tools/query-data.ts', queryData, 'orphanet_query_data', 'registers orphanet_query_data');
}

if (getSchema) {
  assertContains('src/tools/get-schema.ts', getSchema, 'orphanet_get_schema', 'registers orphanet_get_schema');
}
